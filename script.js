...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (currentPage === 'accounts.html') {
        ...
        const renderAccounts = () => {
            ...
            accounts.forEach(account => {
                ...
                card.className = 'premium-card p-6 relative';
                card.innerHTML = `<button class="delete-btn absolute top-3 right-3 text-slate-500 hover:text-red-500" data-id="${account.id}"><i data-lucide="trash-2" class="w-4 h-4"></i></button><h3 class="text-lg font-bold mb-2">${account.name}</h3><div class="text-xs text-[var(--text-secondary)] flex justify-between"><span>Trades: ${tradesForAccount.length}</span><span>Last Activity: ${tradesForAccount.length > 0 ? new Date(tradesForAccount.sort((a,b) => new Date(b.date) - new Date(a.date))[0].date+'T00:00:00').toLocaleDateString() : 'N/A'}</span></div>`;
        ...
        accountForm.addEventListener('submit', (e) => {
            ...
            if (accountName) {
                setStoredData('accounts', [...getStoredData('accounts'), { id: Date.now(), name: accountName }]);
                renderAccounts();
    ...
    if (currentPage === 'journal.html') {
        ...
        const renderJournalEntries = () => {
            const entries = getStoredData('journal').sort((a,b) => new Date(b.date) - new Date(a.date));
            tableBody.innerHTML = '';
        ...
        populateAccountDropdown(formInputs.account); renderJournalEntries();
    }
